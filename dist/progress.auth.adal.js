...
progress.data.AuthenticationProviderAdal = function (uri, adalConfig) {
    ...
    this._initialize(uri, progress.data.Session.AUTH_TYPE_ADAL, { "_loginURI": progress.data.AuthenticationProvider._homeLoginURIBase });
...
(function () {
                            ...
                            if (properties) {
                                if (resource._dataSetName) {
                                    ...
                                    for (var tableName in properties) {
                                        ...
                                        var tableProperties;
                                        if (properties[tableName].items && properties[tableName].items.properties) {
                                        ...
                                        for (var field in tableProperties) {
                                            ...
                                            if (field != '_id') resource.fields[tableName].push(tableProperties[field]);
                                        }
                                    }
                                } else {
                                    ...
                                    for (var field in properties) {
                                        ...
                                        if (field != '_id') resource.fields[tableName].push(properties[field]);
                                    }
                                }
                            } else throw new Error("Error parsing catalog file.");
                        } else resource.fields = null;

                        // Validate relationship property
                        if (resource.relations instanceof Array && resource.relations[0] && resource.relations[0].RelationName) {
                            throw new Error("Relationship properties in catalog must begin with lowercase.");
                        }
                        // Process operations
                        resource.generic = {};
                            ...
                            for (var idx = 0; idx < resource.operations.length; idx++) {
                                ...
                                var func = function fn(object, async) {
                                    ...
                                    if (object) {
                                        ...
                                        var objParam;
                                        if (object instanceof XMLHttpRequest || object.constructor && object.constructor.name === "XMLHttpRequest") {
                                            jsdo = object.jsdo;
                                            xhr = object;
                                            ...
                                            switch (fn.definition.params[i].type) {
                                                ...
                                                case 'RESPONSE_BODY,REQUEST_BODY':
                                                    if (xhr && !reqBody) {
                                                        ...
                                                        if (isInvoke && fn.definition.params[i].xType && "DATASET,TABLE".indexOf(fn.definition.params[i].xType) != -1) {
                                                            var unwrapped = jsdo._resource.service.settings && jsdo._resource.service.settings.unwrapped;
                                                            if (unwrapped) {
                                                                // Remove extra level if found
                                                                if ((typeof reqParam === "undefined" ? "undefined" : _typeof(reqParam)) == 'object' && Object.keys(reqParam).length == 1 && _typeof(reqParam[name]) == 'object') reqParam = reqParam[name];
                                                            } else {
                                                                // Add extra level if not found
                                                                if ((typeof reqParam === "undefined" ? "undefined" : _typeof(reqParam)) == 'object' && typeof reqParam[name] == 'undefined') {
                                                                    reqParam = {};
                                    ...
                                    if (request.deferred === undefined && typeof $ == 'function' && typeof $.Deferred == 'function') {
                                        deferred = $.Deferred();
        ...
        */
        this.setContextProperty = function (propertyName, propertyValue) {
            if (arguments.length < 2) {
                ...
                throw new Error(progress.data._getMsgText("jsdoMSG122", 'Session', 'setContextProperty', 2));
            }
            if (arguments.length !== 2) {
            ...
            if (typeof propertyName !== "string") {
                ...
                throw new Error(progress.data._getMsgText("jsdoMSG121", 'Session', 1, 'string', 'setContextProperty'));
            }
        ...
        this.setContext = function (context) {
            ...
            if ((typeof context === "undefined" ? "undefined" : _typeof(context)) == "object") {
                ...
                // {1}: Parameter {1} must be of type {3} in {4} call.
                throw new Error(progress.data._getMsgText("jsdoMSG121", 'Session', 1, 'Object', 'setContextProperty'));
            }
            contextString = null; // must be updated on next get of this.contextHeader
        };
    ...
    progress.data.Session = function Session(options) {
        ...
        var that = this,
            jsdosession,
            // "backpointer" if this Session is being used by a JSDOSession
        isUserAgentiOS = false,
            // checked just below this var statement
        isFirefox = false,
            // checked just below this var statement
        isEdge = false,
            // checked just below this var statement
        isIE = false,
            // checked just below this var statement
        canPassCredentialsToOpenWithCORS = false,
            // False will always work if creds are correct
        defaultiOSBasicAuthTimeout = 4000,
            deviceIsOnline = true,
            // online until proven offline
        restApplicationIsOnline = false,
            // was the Mobile Web Application that this Session object
        ...
        if (defPropSupported) {
            ...
            Object.defineProperty(this, 'connected', {
                get: function get() {
                    return this.loginResult === progress.data.Session.LOGIN_SUCCESS && restApplicationIsOnline && deviceIsOnline;
                },
                enumerable: true
            });
            ...
            Object.defineProperty(this, "_isInvalidated", {
                get: function get() {
            ...
            Object.defineProperty(this, "_authProvider", {
                ...
                set: function set(newval) {
                    if (_authProvider) {
                        throw new Error("Internal Error setting Session._authProvider. '" + "The property has already been set.");
                    } else {
        ...
        function storeSessionInfo(infoName, value) {
            var key;
            if (that.loginResult === progress.data.Session.LOGIN_SUCCESS && (typeof sessionStorage === "undefined" ? "undefined" : _typeof(sessionStorage)) === 'object' && _storageKey) {
        ...
        function retrieveSessionInfo(infoName) {
            var key,
                jsonStr,
                value = null;
            if ((typeof sessionStorage === "undefined" ? "undefined" : _typeof(sessionStorage)) === 'object' && _storageKey) {
                key = _storageKey;
                if (infoName) {
                    key = key + "." + infoName;
                }
        ...
        function setSessionInfoFromStorage(key) {
            var authproviderInitObject, authProvider;
        ...
        this._openRequest = function (xhr, verb, url, async, callback) {
            ...
            if (this._authProvider) {
                this._authProvider._openRequestAndAuthorize(xhr, verb, urlPlusCCID, async, afterOpenAndAuthorize);
            } else {
        ...
        this.login = function (serviceURI, loginUserName, loginPassword, loginTarget) {
                ...
                if (arguments[0] && _typeof(arguments[0]) === 'object') {
                    ...
                    */
                    ...
                    if (typeof iOSBasicAuthTimeout === 'undefined') {
                        ...
                        throw new Error(progress.data._getMsgText("jsdoMSG033", 'Session', 'login', 'The iOSBasicAuthTimeout argument was invalid.'));
                    }
                } else {
                    args = arguments;
                }
            ...
            if (this.authenticationModel === progress.data.Session.AUTH_TYPE_ANON || this.authenticationModel === progress.data.Session.AUTH_TYPE_FORM) {
            ...
            try {
                ...
                if (isAsync) {
                    ...
                    if (this.authenticationModel === progress.data.Session.AUTH_TYPE_BASIC && isUserAgentiOS && iOSBasicAuthTimeout > 0) {
        ...
        this.logout = function (args) {
            ...
            if ((typeof args === "undefined" ? "undefined" : _typeof(args)) === 'object') {
                ...
                if (isAsync && typeof isAsync !== 'boolean') {
                    throw new Error(progress.data._getMsgText("jsdoMSG033", "Session", 'logout', 'The async argument was invalid.'));
                }
            ...
            try {
                ...
                xhr._jsdosession = jsdosession; // in case the caller is a JSDOSession
                xhr._deferred = deferred; // in case the caller is a JSDOSession
                if (this.authenticationModel === progress.data.Session.AUTH_TYPE_FORM || this.authenticationModel === progress.data.Session.AUTH_TYPE_BASIC) {
                    if (isAsync) {
                        xhr.onreadystatechange = this._onReadyStateChangeGeneric;
        ...
        this.addCatalog = function (arg1, arg2, arg3, arg4) {
            ...
            function addCatalogAfterOpen() {
                ...
                if (isAsync) {
                    ...
                    if (that.authenticationModel === progress.data.Session.AUTH_TYPE_BASIC && isUserAgentiOS && iOSBasicAuthTimeout) {
                        xhr._requestTimeout = setTimeout(function () {
                            clearTimeout(xhr._requestTimeout);
                            xhr._iosTimeOutExpired = true;
                            xhr.abort();
            ...
            if (arguments.length > 0) {
                if ((typeof arg1 === "undefined" ? "undefined" : _typeof(arg1)) === 'object') {
                    ...
                    if (!catalogURI || typeof catalogURI !== 'string') {
                        throw new Error(progress.data._getMsgText("jsdoMSG033", 'Session', 'addCatalog', 'The catalogURI argument was missing or invalid.'));
                    }
                    catalogUserName = arg1.userName;
                    if (catalogUserName && typeof catalogUserName !== 'string') {
                        throw new Error(progress.data._getMsgText("jsdoMSG033", 'Session', 'addCatalog', 'The catalogUserName argument was invalid.'));
                    }
                    catalogPassword = arg1.password;
                    if (catalogPassword && typeof catalogPassword !== 'string') {
                        throw new Error(progress.data._getMsgText("jsdoMSG033", 'Session', 'addCatalog', 'The catalogPassword argument was invalid.'));
                    }
                    isAsync = arg1.async;
                    if (isAsync && typeof isAsync !== 'boolean') {
                        throw new Error(progress.data._getMsgText("jsdoMSG033", 'Session', 'addCatalog', 'The async argument was invalid.'));
                    }
                    iOSBasicAuthTimeout = arg1.iOSBasicAuthTimeout;
                    if (typeof iOSBasicAuthTimeout === 'undefined') {
                        iOSBasicAuthTimeout = defaultiOSBasicAuthTimeout;
                    } else if (iOSBasicAuthTimeout && typeof iOSBasicAuthTimeout !== 'number') {
                        throw new Error(progress.data._getMsgText("jsdoMSG033", 'Session', 'addCatalog', 'The iOSBasicAuthTimeout argument was invalid.'));
                    }
            ...
            if (progress.data.ServicesManager.getSession(catalogURI) !== undefined) {
                if (isAsync) {
                    ...
                    setTimeout(this._addCatalogComplete, 10, this, progress.data.Session.CATALOG_ALREADY_LOADED, null, xhr);
                    return progress.data.Session.ASYNC_PENDING;
                }
            ...
            if (authProvider) {
                ...
                // should be able to get rid of this if we do away with synchronous (old Session API) support
        ...
        this._processAddCatalogResult = function (xhr) {
            ...
            if (_catalogHttpStatus == 200 || _catalogHttpStatus === 0 && xhr.responseText) {
                ...
                for (var i = 0; i < servicedata.length; i++) {
                    ...
                    if (servicedata[i].settings && servicedata[i].settings.useXClientProps && !theSession.xClientProps) {
                        console.warn("Catalog warning: Service settings property 'useXClientProps' " + "is true but 'xClientProps' property has not been set.");
                    }
                }
                ...
                throw new Error("Error retrieving catalog '" + catalogURI + "'. Http status: " + _catalogHttpStatus + ".");
            }
        ...
        this._sendPing = function (args) {
            ...
            try {
                if (this._authProvider) {
                    this._authProvider._openRequestAndAuthorize(xhr, 'GET', args.pingURI, args.async, sendPingAfterOpen);
                } else {
        ...
        this._setXHRCredentials = function (xhr, verb, uri, userName, password, async) {
            ...
            if (userName && this.authenticationModel === progress.data.Session.AUTH_TYPE_BASIC) {
        ...
        this._addCCIDtoURL = function (url) {
            var urlPart1, urlPart2, jsessionidStr, index;
        ...
        function _findStringInResponseHTML(xhr, searchString) {
            ...
            if (contentType.indexOf("text/html") >= 0 && xhr.responseText.indexOf(searchString) >= 0) {
                return true;
            }
        ...
        function handleJSONLoginResponse(params) {
            ...
            try {
                ...
                if (jsonObject.status_code !== undefined && jsonObject.status_txt !== undefined) {
        ...
        function toggleOnlineState(xhr) {
            ...
            if (pdsession.loginHttpStatus >= 200 && pdsession.loginHttpStatus < 400) {
                ...
                if (pdsession.loginHttpStatus === 0 || pdsession.loginHttpStatus === 400 || pdsession.loginHttpStatus === 410) {
                    setRestApplicationIsOnline(false);
                    setLoginResult(progress.data.AuthenticationProvider._getAuthFailureReason(xhr), pdsession);
                }
    ...
    */
    progress.data.JSDOSession = function JSDOSession(options) {
        ...
        function onAfterAddCatalog(pdsession, result, errorObject, xhr) {
            ...
            if (xhr && xhr._deferred) {
                ...
                if (result !== progress.data.Session.SUCCESS && result !== progress.data.Session.CATALOG_ALREADY_LOADED) {
                ...
                if (deferred._numCatalogsProcessed === deferred._numCatalogs) {
                    ...
                    settlePromise(xhr._deferred, fulfill, settleResult, xhr._deferred._results);
                }
            }
        }
        ...
        this.login = function (username, password, options) {
            ...
            function callIsAuthorized() {
                that.isAuthorized().then(function (jsdosession, result, info) {
            ...
            _pdsession._authProvider.logout().then(function () {
        ...
        this.addCatalog = function (catalogURI, unameOrOpts, password, opts) {
            ...
            if (typeof catalogURI === "string") {
                ...
                throw new Error(progress.data._getMsgText("jsdoMSG033", "JSDOSession", "addCatalog", "The first argument must be a string or an array of strings specifying the URI of the catalog."));
            }
            ...
            if (unameOrOpts) {
                ...
                } else if ((typeof unameOrOpts === "undefined" ? "undefined" : _typeof(unameOrOpts)) === "object") {
                    options = unameOrOpts;
                } else {
                    // JSDOSession: Argument 2 must be of type object in addCatalog call.
                    throw new Error(progress.data._getMsgText("jsdoMSG121", "JSDOSession", "2", "object", "addCatalog"));
                }
            }
        ...
        this.logout = function () {
            ...
            if (this.authenticationModel === progress.data.Session.AUTH_TYPE_SSO) {
                ...
                // Please use the AuthenticationProvider object instead.
                throw new Error(progress.data._getMsgText("jsdoMSG057", 'JSDOSession', 'logout()'));
            }
        ...
        this.isAuthorized = function () {
            ...
            if (this.loginResult === progress.data.Session.LOGIN_SUCCESS || this.authProvider) {
                _pdsession._openRequest(xhr, "GET", _pdsession.loginTarget, true, function () {
                    xhr.onreadystatechange = function () {
                        ...
                        if (xhr.readyState === 4) {
                            ...
                            if (xhr.status >= 200 && xhr.status < 300) {
                                deferred.resolve(that, progress.data.Session.SUCCESS, info);
                            } else {
                                if (xhr.status === 401) {
        ...
        if (arguments.length > 0 && _typeof(arguments[0]) === 'object') {
            ...
            if (options.serviceURI && typeof options.serviceURI === "string") {
                ...
                throw new Error(progress.data._getMsgText("jsdoMSG033", "JSDOSession", "the constructor", "The options parameter must include a 'serviceURI' property that is a string."));
            }

            if (options.authenticationModel) {
                if (typeof options.authenticationModel !== "string") {
                    throw new Error(progress.data._getMsgText("jsdoMSG033", "JSDOSession", "the constructor", "The authenticationModel property of the options parameter must be a string."));
                }
            ...
            if (options.authProvider) {
                if (_typeof(options.authProvider) !== 'object') {
                    // JSDOSession: The 'options' parameter passed to the 'constructor' function
                    //              has an invalid value for the 'authProvider' property.
                    throw new Error(progress.data._getMsgText("jsdoMSG502", "JSDOSession", "options", "constructor", "authProvider"));
                }

                if (options.authProvider.authenticationModel !== progress.data.Session.AUTH_TYPE_FORM_SSO && options.authProvider.authenticationModel !== options.authenticationModel || options.authProvider.authenticationModel === progress.data.Session.AUTH_TYPE_FORM_SSO && options.authenticationModel !== progress.data.Session.AUTH_TYPE_SSO) {
                    ...
                    throw new Error(progress.data._getMsgText("jsdoMSG059", "JSDOSession", options.authProvider.authenticationModel, options.authenticationModel));
                }
                ...
                if (typeof options.authProvider.hasClientCredentials === 'function') {
                    ...
                    throw new Error(progress.data._getMsgText("jsdoMSG505", "JSDOSession", "AuthenticationProvider", "hasClientCredentials"));
                }
            ...
            throw new Error(progress.data._getMsgText("jsdoMSG033", "JSDOSession", "the constructor", "The options argument was missing or invalid."));
        }
    ...
    progress.data.getSession = function (options) {
        ...
        function sessionRejectHandler(originator, result, info) {
            ...
            if (authProvider && authProvider.hasClientCredentials()) {
                authProvider.logout().always(function () {
        ...
        function loginHandler(provider) {
            ...
            try {
                ...
                try {
                    jsdosession.isAuthorized().then(function () {
                        return jsdosession.addCatalog(options.catalogURI);
                    }, sessionRejectHandler).then(function (jsdosession, result, info) {
                        deferred.resolve(jsdosession, progress.data.Session.SUCCESS);
                    }, sessionRejectHandler);
                } catch (e) {
                    sessionRejectHandler(jsdosession, progress.data.Session.GENERAL_FAILURE, { errorObject: e });
                }
            } catch (e) {
                sessionRejectHandler(jsdosession, progress.data.Session.GENERAL_FAILURE, { errorObject: e });
            }
        ...
        function callLogin(provider) {
            ...
            if (typeof options.loginCallback !== 'undefined') {
                options.loginCallback().then(function (result) {
                    try {
                        provider.login(result.username, result.password).then(loginHandler, sessionRejectHandler);
                    } catch (e) {
                        sessionRejectHandler(provider, progress.data.Session.GENERAL_FAILURE, {
                            errorObject: e
                        });
                    }
                ...
                try {
                    provider.login(options.username, options.password).then(loginHandler, sessionRejectHandler);
                } catch (e) {
                    sessionRejectHandler(provider, progress.data.Session.GENERAL_FAILURE, {
                        errorObject: e
                    });
                ...
                errorObject = new Error(progress.data._getMsgText("jsdoMSG052", "getSession()"));
                sessionRejectHandler(provider, progress.data.Session.AUTHENTICATION_FAILURE, {
        ...
        if ((typeof options === "undefined" ? "undefined" : _typeof(options)) !== 'object') {
            ...
            throw new Error(progress.data._getMsgText("jsdoMSG503", "getSession()", "options", "object"));
        }

        if (typeof options.loginCallback !== 'undefined' && typeof options.loginCallback !== 'function') {
            // getSession(): 'options.loginCallback' must be of type 'function'
            throw new Error(progress.data._getMsgText("jsdoMSG503", "getSession()", "options.loginCallback", "function"));
        }
        ...
        try {
            ...
            if (options.authenticationURI) {
                ...
                if (typeof authProviderInitObject.authenticationModel !== "string") {
                    ...
                    throw new Error(progress.data._getMsgText("jsdoMSG502", "progress.data.getSession", "object", "getSession", "authProviderAuthenticationModel"));
                }
            } else {
            ...
            // throw error;
            sessionRejectHandler(null, progress.data.Session.GENERAL_FAILURE, {
                errorObject: error
            });
        }
    ...
    progress.data.invalidateAllSessions = function () {
        ...
        $.when.apply($, invalidatePromises).then(function () {
...
(function () {
    ...
    msg.msgs.jsdoMSG026 = "JSDO: saveChanges requires a " + "CREATE, UPDATE, DELETE or SUBMIT operation to be defined.";
    ...
    msg.msgs.jsdoMSG058 = "{1}: Cannot pass username and password to addCatalog when authenticationModel " + "is sso. Pass an AuthenticationProvider instead.";
    msg.msgs.jsdoMSG059 = "{1}: Error in constructor. The authenticationModels of the " + "AuthenticationProvider ({2}) and the JSDOSession ({3}) were not compatible.";
    msg.msgs.jsdoMSG060 = "AuthenticationProvider: AuthenticationProvider is no longer logged in. " + "Tried to refresh SSO token but failed due to authentication error at token server.";
    ...
    msg.msgs.jsdoMSG119 = "JSDO: A mappingType is expected when 'capabilities' is set." + " Please specify a plugin (ex: JFP).";
    ...
    msg.msgs.jsdoMSG124 = "JSDO: autoApplyChanges is not supported for saveChanges(true) " + "with a temp-table. Use jsdo.autoApplyChanges = false.";
    ...
    msg.msgs.jsdoMSG502 = "{1}: The '{2}' parameter passed to the '{3}' function has an invalid value for " + "its '{4}' property.";
    ...
    msg.msgs.jsdoMSG508 = "JSDOSession: If a JSDOSession object is using the SSO authentication model, " + "the options object passed to its constructor must include an authProvider property.";
    msg.msgs.jsdoMSG509 = "progress.data.getSession: If the authenticationModel is AUTH_TYPE_SSO, " + "authenticationURI and authProviderAuthenticationModel are required parameters.";
    msg.msgs.jsdoMSG510 = "{1}: This session has been invalidated and cannot be used.";
    msg.msgs.jsdoMSG511 = "JSDOSession: addCatalog() can only be called if an AuthenticationProvider was passed as an argument or " + "connect() has been successfully called.";
    ...
    progress.data.JSTableRef = function JSTableRef(jsdo, tableName) {
            ...
            if (prodsBeforeData && prodsBeforeData[this._name]) {
                ...
                for (i = 0; i < prodsBeforeData[this._name].length; i++) {
                    ...
                    if (record["prods:rowState"] == "deleted") {
                        ...
                        if (record._id === undefined) record._id = progress.data._getNextId();
                        var copy = {};
                        this._jsdo._copyRecord(this._tableRef, record, copy);
                    ...
                    switch (record["prods:rowState"]) {
                        ...
                        case "modified":
                            ...
                            if (recordId !== undefined) {
                                ...
                                var copy = {};
                                this._jsdo._copyRecord(this._tableRef, beforeRecord, copy);
                                this._jsdo._deleteProdsProperties(copy);
        ...
        this.getData = function (params) {
            ...
            if (this._hasEmptyBlocks) {
                ...
                if (numEmptyBlocks * 100 / this._data.length >= PROGRESS_JSDO_PCT_MAX_EMPTY_BLOCKS) this._needCompaction = true;
        ...
        this._recFromDataObject = function (dataObject) {
            ...
            if (dataObject) {
                if (this._jsdo._dataSetName) {
                    if (dataObject[this._jsdo._dataSetName]) data = dataObject[this._jsdo._dataSetName][this._name];
                } else {
                    if (this._jsdo._dataProperty) {
                        if (dataObject[this._jsdo._dataProperty]) data = dataObject[this._jsdo._dataProperty];
        ...
        this._add = function (values, trackChanges, setWorkingRecord) {
                        ...
                        if (typeof value != "undefined") {
                            if (!this._fields[name.toLowerCase()]) {
                                ...
                                if (typeof value == 'string' && schema[i].items.type != 'string') {
                                    value = this._jsdo._convertType(value, schema[i].items.type, null);
                                }
            ...
            if (this._jsdo.useRelationships && this._relationship && this._parent) {
                if (this._jsdo._buffers[this._parent].record) {
                    for (var j = 0; j < this._relationship.length; j++) {
                        record[this._relationship[j].childFieldName] = this._jsdo._buffers[this._parent].record.data[this._relationship[j].parentFieldName];
                    }
                } else throw new Error(msg.getMsgText("jsdoMSG002", this._parent));
            }
            ...
            if (this.autoSort && this._sortRecords && (this._sortFn !== undefined || this._sortObject.sortFields !== undefined)) {
                if (this._needsAutoSorting) {
                    ...
                    for (var i = 0; i < this._data.length; i++) {
                        ...
                        var ret = this._sortFn ? this._sortFn(record, this._data[i]) : this._compareFields(record, this._data[i]);
                        if (ret == -1) break;
                    }
            ...
            if (setWorkingRecord) this._setRecord(jsrecord, true);
        ...
        this._getRelatedData = function (jsrecord) {
            ...
            if (typeof jsrecord == 'undefined') {
                if (this._jsdo.useRelationships && this._relationship && this._parent) {
                    ...
                    if (!jsrecord) throw new Error(msg.getMsgText("jsdoMSG002", this._parent));
                }
            }
            if (jsrecord) {
                ...
                for (var i = 0; i < this._data.length; i++) {
                    ...
                    if (match) data.push(this._data[i]);
                }
                        ...
                        if (childBuf._isNested) {
                            ...
                            for (var k = 0; k < childBuf._data.length; k++) {
                                ...
                                for (var m = 0; m < childBuf._relationship.length; m++) {
                                    match = parentRecord[childBuf._relationship[m].parentFieldName] == childRecord[childBuf._relationship[m].childFieldName];
                                    if (!match) break;
                                }
                                if (match) {
                ...
                if (this._jsdo._buffers[this._parent].record) {
                    ...
                    for (var i = 0; i < this._data.length; i++) {
                        ...
                        for (var j = 0; j < this._relationship.length; j++) {
                            ...
                            match = this._jsdo._buffers[this._parent].record.data[parentFieldName] == this._data[i][childFieldName];
                            if (!match) break;
                        }
                        if (match) {
        ...
        this.assign = function (values) {
            ...
            } else throw new Error(msg.getMsgText("jsdoMSG002", this._name));
        };
        ...
        this._findById = function (id, setWorkingRecord) {
            ...
            if (id && this._index[id]) {
                ...
                if (setWorkingRecord) this._setRecord(this.record);
                return this.record;
            }

            if (setWorkingRecord) this._setRecord(null);
            return null;
        };
        ...
        this.foreach = function (fn) {
            ...
            if (this._needCompaction) this._compact();
            ...
            for (var i = 0; i < keyFields.length; i++) {
                ...
                if (!jsdo[tableName].caseSensitive) {
                    ...
                    if (field && field.type == "string") {
                        if (value1 !== undefined && value1 !== null) value1 = value1.toUpperCase();
                        if (value2 !== undefined && value2 !== null) value2 = value2.toUpperCase();
                    }
                }
        ...
        this._getKey = function (record, keyFields) {
            ...
            for (var i = 0; i < keyFields.length; i++) {
                ...
                if (!jsdo[tableName].caseSensitive) {
                    field = jsdo[tableName]._fields[fieldName.toLowerCase()];
                    if (field && field.type == "string") {
                        if (value !== undefined && value !== null) value = value.toUpperCase();
                    }
                }
        ...
        this._getCompareFn = function (sortObject) {
            if (typeof sortObject == 'function') {
                ...
                for (var i = 0; i < sortFields.length; i++) {
                    ...
                    if (value1 > value2 || value1 === undefined || value1 === null) return sortAscending[i] ? 1 : -1;else if (value1 < value2 || value2 === undefined && value2 === null) return sortAscending[i] ? -1 : 1;
                }
                return 0;
            };
        };
        ...
        if (typeof Object.defineProperty == 'function') {
            ...
            Object.defineProperty(this, "caseSensitive", {
                ...
                set: function set(value) {
                    ...
                    } else this._caseSensitive = false;
                    if (this.autoSort && this._sortObject.sortFields && !this._sortFn) {
                        this._sort();
                        this._createIndex();
                    }
                },
            ...
            if (sortFields instanceof Array) {
                ...
                for (var i = 0; i < sortObject.sortFields.length; i++) {
                    ...
                    if (fieldName != "_id" && this._fields) {
                        ...
                        if (field) {
                            if (field.type == "array") throw new Error(msg.getMsgText("jsdoMSG030", "data type found in sort", "scalar field"));
                            fieldName = field.name;
                        } else throw new Error(msg.getMsgText("jsdoMSG031", fieldName));
                    }
        ...
        this.setSortFields = function (sortFields) {
            ...
            } else throw new Error(msg.getMsgText("jsdoMSG024", "JSDO", "setSortFields()"));
        };
        ...
        this.sort = function (arg1) {
            ...
            if (arguments.length !== 1 || !(arg1 instanceof Array) && typeof arg1 != 'function') {
                throw new Error(msg.getMsgText("jsdoMSG024", "JSDO", "sort()"));
            }

            if (arg1 instanceof Array) {
                var sortObject = this._processSortFields(arg1);
                if (sortObject.sortFields && sortObject.sortFields.length > 0) this._sort(sortObject);
            } else {
                this._sort(arg1);
            }
            this._createIndex();
        ...
        this._sort = function (arg1) {
            if (arguments.length === 0 && (!this.autoSort || this._sortFn === undefined && this._sortObject.sortFields === undefined)) return;
            ...
            if (arguments.length === 0) {
                ...
                if (this.autoSort) this._needsAutoSorting = true;
            }
        };
        ...
        this._applyChanges = function () {
            for (var id in this._beforeImage) {
                ...
                if (this._beforeImage[id] === null) {
                    var jsrecord = this._findById(id, false);
                    if (jsrecord !== null) {
                        if (jsrecord.data._rejected || jsrecord.data._errorString !== undefined) {
                            this._jsdo._undoCreate(this, id);
                        } else {
                ...
                // Update
                        ...
                        if (jsrecord !== null) {
                            ...
                            // Record not present in JSDO memory
                            // Delete after Update
                            if (this._beforeImage[id]._rejected || this._beforeImage[id]._errorString !== undefined) {
        ...
        this.acceptRowChanges = function () {
            if (this.record) return this.record.acceptRowChanges();
    ...
    progress.data.JSRecord = function JSRecord(tableRef, record) {
        ...
        this._saveBeforeImageUpdate = function () {
            ...
            if (this._tableRef._beforeImage[this.data._id] === undefined) {
                ...
                var copy = {};
                this._tableRef._jsdo._copyRecord(this._tableRef, this.data, copy);
        ...
        this._sortRecord = function (fields) {
            ...
            if (this._tableRef.autoSort && this._tableRef._sortRecords && (this._tableRef._sortFn !== undefined || this._tableRef._sortObject.sortFields !== undefined)) {

                if (this._tableRef._sortObject.fields) {
                    if (typeof fields == 'string') {
                        if (this._tableRef._sortObject.fields[fields] === undefined) return; // Only sort records if the the specified field is in the sort fields
                        ...
                        if (!found) return; // Only sort records if the the specified fields are in the sort fields
                    }
                }
                ...
                if (this._tableRef._needsAutoSorting) {
                    ...
                    for (var i = 0; i < this._tableRef._data.length; i++) {
                        ...
                        var ret = this._tableRef._sortFn ? this._tableRef._sortFn(record, this._tableRef._data[i]) : this._tableRef._compareFields(record, this._tableRef._data[i]);
                        if (ret == -1) break;
                    }
        ...
        this.assign = function (record) {
            if (record === undefined) throw new Error(msg.getMsgText("jsdoMSG024", "JSDO", "assign() or update()"));
                ...
                for (i = 0; i < schema.length; i += 1) {
                    ...
                    if (typeof value != "undefined") {
                        if (typeof value == 'string' && schema[i].type != 'string') {
                            value = this._tableRef._jsdo._convertType(value, schema[i].type, schema[i].items ? schema[i].items.type : null);
                        }
                        ...
                        for (j = 0; j < schema[i].maxItems; j += 1) {
                            ...
                            if (typeof value != "undefined") {
                                ...
                                if (!this._tableRef._fields[name.toLowerCase()]) {
                                    if (typeof value == 'string' && schema[i].items.type != 'string') {
                                        value = this._tableRef._jsdo._convertType(value, schema[i].items.type, null);
                                    }
    ...
    progress.data.JSDO = function JSDO(resNameOrParmObj, serviceName) {
        ...
        this._defineProperty = function (tableName, fieldName) {
            Object.defineProperty(this._buffers[tableName], fieldName, {
                get: function fnGet() {
                    var name, index, element, fieldInfo;
                    if (this.record) {
        ...
        if (this._events) {
            ...
            for (var prop in this._events) {
                ...
                evt.forEach(function (el) {
                    ...
                    if (el.scope && _typeof(el.scope) !== 'object') {
        ...
        if (this.name) {
            ...
            if (this._resource) {
                if (!this.url) this.url = this._resource.url;
                ...
                if (!this._dataProperty && this._resource.dataProperty) this._dataProperty = this._resource.dataProperty;
                ...
                if (!this._dataSetName) {
                    ...
                    if (tableName) this[tableName] = this._buffers[tableName];
                }
                ...
                this._hasCUDOperations = this._resource.generic["create"] !== undefined || this._resource.generic["update"] !== undefined || this._resource.generic["delete"] !== undefined;
        ...
        for (var buf in this._buffers) {
            ...
            if (!this._defaultTableRef) this._defaultTableRef = this._buffers[buf];
            this._numBuffers++;
        }
        if (this._numBuffers != 1) this._defaultTableRef = null;else {
            // record is used to represent the current record for a table reference
        ...
        this._properties = {};
        if (typeof Object.defineProperty == 'function') {
            Object.defineProperty(this, "this._properties", {
                get: function get() {
        ...
        if (this._resource && this._resource.fields) {
            for (var buf in this._buffers) {
                ...
                for (var i = 0; i < fields.length; i++) {
                    ...
                    if (typeof fields[i].origName !== "undefined") {
                        if (typeof fields[i].origName !== "string" || fields[i].origName.trim() === "") {
                            throw new Error(msg.getMsgText("jsdoMSG504", "JSDO", "Field '" + fields[i].name + "' in resource '" + this._resource.name + "'", "origName"));
                        }
                    }
                }
            ...
            if (this._defaultTableRef && !this._defaultTableRef._schema && this._resource.fields[""]) {
        ...
        if (this._numBuffers > 1) {
            for (var buf in this._buffers) {
                ...
                for (var i = 0; i < this._buffers[buf]._schema.length; i++) {
                    ...
                    if (field.items && field.type == "array" && field.items.$ref) {
                ...
                if (found) this._buffers[buf]._schema = fields;
            }
        }
        ...
        if (this._resource && this._resource.relations) {
            for (var i = 0; i < this._resource.relations.length; i++) {
                ...
                if (relationship.childName && relationship.parentName && relationship.childName !== relationship.parentName) {
                    ...
                    if (relationship.relationFields instanceof Array) {
                        for (var j = 0; j < relationship.relationFields.length; j++) {
                            ...
                            if (this._buffers[relationship.parentName]._fields) {
                                ...
                                } else throw new Error(msg.getMsgText("jsdoMSG010", fieldName));
                            }
        ...
        this._getDefaultValue = function (field) {
            ...
            if (field.type === "string" && field.format && field.format.indexOf("date") !== -1 && field["default"]) {
                isDate = true;
            } else if (field.type === "array" && field.ablType && field.ablType.indexOf("DATE") != -1 && field["default"]) {
                isDate = true;
            } else {
        ...
        this._getDataObject = function () {
            ...
            if (this._dataSetName) {
                ...
                } else return this.getData(); // Array
            }
            return dataObject;
        };
        ...
        this._recToDataObject = function (record, includeChildren) {
            if (this._defaultTableRef) return this._defaultTableRef._recToDataObject(record, includeChildren);
            throw new Error(msg.getMsgText("jsdoMSG001", "_recToDataObject()"));
        };

        this._recFromDataObject = function (dataObject) {
            if (this._defaultTableRef) return this._defaultTableRef._recFromDataObject(dataObject);
            throw new Error(msg.getMsgText("jsdoMSG001", "_recFromDataObject()"));
        };

        this.add = function (obj) {
            if (this._defaultTableRef) return this._defaultTableRef.add(obj);
        ...
        this.hasData = function () {
            for (var buf in this._buffers) {
                if (this._buffers[this._buffers[buf]._name].hasData()) return true;
            }
            return false;
        };

        this.getData = function (params) {
            if (this._defaultTableRef) return this._defaultTableRef.getData(params);
            throw new Error(msg.getMsgText("jsdoMSG001", "getData()"));
        };

        this.getSchema = function () {
            if (this._defaultTableRef) return this._defaultTableRef.getSchema();
            throw new Error(msg.getMsgText("jsdoMSG001", "getSchema()"));
        };

        this.findById = function (id) {
            if (this._defaultTableRef) return this._defaultTableRef.findById(id);
        ...
        this.assign = function (values) {
            ...
            } else throw new Error(msg.getMsgText("jsdoMSG001", "assign() or update()"));
        };
        ...
        this.remove = function () {
            ...
            } else throw new Error(msg.getMsgText("jsdoMSG001", "remove()"));
        };

        this.getId = function () {
            if (this._defaultTableRef) return this._defaultTableRef.getId();
            throw new Error(msg.getMsgText("jsdoMSG001", "getId()"));
        };

        // getErrors() - JSDO
        this.getErrors = function () {
            if (this._defaultTableRef) return this._defaultTableRef.getErrors();
            throw new Error(msg.getMsgText("jsdoMSG001", "getErrors()"));
        };

        this.getErrorString = function () {
            if (this._defaultTableRef) return this._defaultTableRef.getErrorString();
         ...
         */
        this.find = function (fn) {
            if (this._defaultTableRef) return this._defaultTableRef.find(fn);
            throw new Error(msg.getMsgText("jsdoMSG001", "find()"));
        };

        this.foreach = function (fn) {
            if (this._defaultTableRef) return this._defaultTableRef.foreach(fn);
            throw new Error(msg.getMsgText("jsdoMSG001", "foreach()"));
        };

        this.setSortFields = function (sortFields) {
            if (this._defaultTableRef) return this._defaultTableRef.setSortFields(sortFields);
            throw new Error(msg.getMsgText("jsdoMSG001", "setSortFields()"));
        };

        this.setSortFn = function (fn) {
            if (this._defaultTableRef) return this._defaultTableRef.setSortFn(fn);
            throw new Error(msg.getMsgText("jsdoMSG001", "setSortFn()"));
        };

        this.sort = function (arg1) {
            if (this._defaultTableRef) return this._defaultTableRef.sort(arg1);
        ...
        this._setAllRecordsRejected = function (param) {
            var changes, hasErrors, hasRejected, hasCommittedRecords, i;
            ...
            if (param instanceof Object) {
                if (param instanceof Array) {
                    ...
                    if (param.operations instanceof Array) {
                        if (param.operations.length > 0 && !param.operations[0].success) {
         ...
         */
        this._execGenericOperation = function (operation, objParam, request, onCompleteFn, onSuccessFn, onErrorFn) {
        ...
        this._convertField = function (value, ablType) {
            ...
            try {
                ...
                throw new Error(msg.getMsgText("jsdoMSG000", "Error in _convertField for value: " + value + ". " + e.message));
            }
        ...
        this._convertDate = function (value, ablType) {
            ...
            if (ablType === "DATE" || ablType === "DATETIME") {
                result = progress.util._pad(value.getFullYear(), 4) + '-' + progress.util._pad(value.getMonth() + 1) + '-' + progress.util._pad(value.getDate());

                if (ablType === "DATETIME") {
                    result = result + "T" + progress.util._pad(value.getHours()) + ":" + progress.util._pad(value.getMinutes()) + ":" + progress.util._pad(value.getSeconds()) + "." + progress.util._pad(value.getMilliseconds(), 3);
                }
            }
        ...
        this._syncTableRef = function (operation, tableRef, batch) {
            var rowData, requestData, jsonObject;
            ...
            switch (operation) {
                case progress.data.JSDO._OP_CREATE:
                    for (var i = 0; i < tableRef._added.length; i++) {
                        ...
                        this._execGenericOperation(progress.data.JSDO._OP_CREATE, jsonObject, request, this._createComplete, this._createSuccess, this._createError);
                    }
                    break;
                case progress.data.JSDO._OP_UPDATE:
                    for (var id in tableRef._changed) {
                        ...
                        if (!useBeforeImageFormat) {
                            if (this._resource.service && this._resource.service.settings && this._resource.service.settings.sendOnlyChanges) {
                        ...
                        this._execGenericOperation(progress.data.JSDO._OP_UPDATE, jsonObject, request, this._updateComplete, this._updateSuccess, this._updateError);
                    }
                    break;
            }
            ...
            for (var i = 0; i < tableRef._children.length; i++) {
                ...
                this._syncTableRef(operation, this._buffers[childTableName], batch);
            }
            ...
            if (operation == progress.data.JSDO._OP_DELETE) {
                for (var i = 0; i < tableRef._deleted.length; i++) {
                    ...
                    this._execGenericOperation(progress.data.JSDO._OP_DELETE, jsonObject, request, this._deleteComplete, this._deleteSuccess, this._deleteError);
                }
            }
        };
        ...
        this._syncDataSetForCUD = function () {
            ...
            for (var buf in this._buffers) {
                ...
                this._syncTableRef(progress.data.JSDO._OP_DELETE, tableRef, batch);
            }
            ...
            for (var buf in this._buffers) {
                var tableRef = this._buffers[buf];
                this._syncTableRef(progress.data.JSDO._OP_CREATE, tableRef, batch);
            }
            ...
            for (var buf in this._buffers) {
                var tableRef = this._buffers[buf];
                this._syncTableRef(progress.data.JSDO._OP_UPDATE, tableRef, batch);
            }
        ...
        this._syncSingleTable = function () {
            ...
            if (!this._async) fireAfterSaveChanges = true;
        ...
        this._syncDataSetForSubmit = function (request) {
            ...
            this._execGenericOperation(progress.data.JSDO._OP_SUBMIT, jsonObject, request, completeFn, successFn, errorFn);
            ...
            if (hasChanges) {
                ...
                for (i = 0; i < tableRef._added.length; i++) {
                    ...
                    if (jsrecord) {
                        if (!tableRef._processed[jsrecord.data._id]) {
                            this._addRowToTTChangeSet(tableRef, jsrecord, tempTableJsonObject, request, "beforeCreate");
                        }
                    }
                }
                ...
                for (id in tableRef._changed) {
                    if (tableRef._changed.hasOwnProperty(id)) {
                        ...
                        if (jsrecord) {
                            if (!tableRef._processed[jsrecord.data._id]) {
                                this._addRowToTTChangeSet(tableRef, jsrecord, tempTableJsonObject, request, "beforeUpdate");
                            }
                        }
                    }
                }
        ...
        this._addRecordsToObject = function (tableRef, dataSetJsonObject) {
            ...
            if (tableRef._data.length > 0 && !dataSetJsonObject[tableRef._name]) dataSetJsonObject[tableRef._name] = [];
        ...
        this._doesRecordHaveCreateBIData = function (tableRef, id) {
            for (var i = 0; i < tableRef._added.length; i++) {
                if (tableRef._added[i] === id) return true;
            }
        ...
        this._addDeletesToChangeSet = function (tableRef, dataSetJsonObject, request) {
            ...
            for (var i = 0; i < tableRef._deleted.length; i++) {
                ...
                if (jsrecord.data && jsrecord.data._id !== undefined && tableRef._beforeImage[jsrecord.data._id] === null) {
        ...
        this.addRecords = function (jsonObject, addMode, keyFields, trackChanges, isInvoke) {
            if (this.isDataSet()) {
                ...
                if (addMode != progress.data.JSDO.MODE_EMPTY) {
                    if (Object.keys(jsonObject).length === 0) throw new Error(msg.getMsgText("jsdoMSG006"));
                }
                ...
                try {
                    for (var buf in this._buffers) {
                        ...
                        if (jsonObject[this._buffers[buf]._name]) this._addRecords(this._buffers[buf]._name, jsonObject, addMode, keyFields, trackChanges, isInvoke);else if (addMode == progress.data.JSDO.MODE_EMPTY) {
        ...
        this._copyRecord = function (tableRef, source, target, onlyChangesRecord) {
            for (var field in source) {
                ...
                if (onlyChangesRecord !== undefined) {
                    if (source[field] == onlyChangesRecord[field]) continue;
                }
                ...
                if (source[field] === undefined || source[field] === null) {
                    ...
                    target[field] = source[field];
                } else if (_typeof(source[field]) === 'object') {
        ...
        this._addRecords = function (tableName, jsonObject, addMode, keyFields, trackChanges, isInvoke) {
            ...
            if (typeof tableName != 'string') throw new Error(msg.getMsgText("jsdoMSG020"));
            if (!addMode) throw new Error(msg.getMsgText("jsdoMSG021"));
            ...
            if (!keyFields) keyFields = [];else {
                if (!(keyFields instanceof Array) && (typeof keyFields === "undefined" ? "undefined" : _typeof(keyFields)) == 'object') {
            ...
            if (tableName) {
                if (!(jsonObject instanceof Array)) {
                    ...
                    if (this.isDataSet()) {
                        if (jsonObject[this._dataSetName]) data = jsonObject[this._dataSetName][tableName];else if (jsonObject[tableName]) data = jsonObject[tableName];
                    } else {
                        if (this._dataProperty) data = jsonObject[this._dataProperty];else if (jsonObject.data) data = jsonObject.data;
                    }

                    if (data instanceof Array) {
                        saveJsonObject = jsonObject;
                        jsonObject = data;
                    } else if (addMode == progress.data.JSDO.MODE_EMPTY && (typeof jsonObject === "undefined" ? "undefined" : _typeof(jsonObject)) == 'object' && Object.keys(jsonObject).length === 0) {
                    ...
                    else if (addMode == progress.data.JSDO.MODE_REPLACE && (typeof jsonObject === "undefined" ? "undefined" : _typeof(jsonObject)) == 'object' && beforeImageJsonObject) {
                            jsonObject = [];
                        }
                }
                ...
                try {
                    ...
                    if (keyFields.length === 0 || addMode == progress.data.JSDO.MODE_EMPTY) {
                        ...
                        } else tmpIndex = null; // Do not use an index
                        ...
                        for (var i = 0; i < jsonObject.length; i++) {
                            ...
                            if (match) {
                                ...
                                if (checkBeforeImage && jsonObject[i]["prods:id"] !== undefined && typeof beforeRecord !== 'undefined') {
                                    ...
                                    if (beforeRecord) {
                                        var beforeObject = this._getBeforeRecordFromObject(jsonObject[i], beforeImageJsonObject, tableName);
                                        if (beforeObject) isBeforeSame = this._sameData(beforeObject, beforeRecord);
                                    }

                                    if (!isAfterSame || !isBeforeSame) throw new Error(msg.getMsgText("jsdoMSG032"));
                                }
                                ...
                                switch (addMode) {
                                    ...
                                    case progress.data.JSDO.MODE_REPLACE:
                                        ...
                                        if (jsonObject[i]._id === undefined) jsonObject[i]._id = record._id;
                                        this._copyRecord(this._buffers[tableName], jsonObject[i], record);
        ...
        this._sameData = function (record1, record2) {
            ...
            for (var fieldName in record1) {
                if (fieldName.substring(0, 5) != "prods" && fieldName != "_id") {
                    ...
                    if (value1 > value2 || value1 === null) return false;else if (value1 < value2 || value2 === null) return false;
                }
            }
        ...
        this._mergeRead = function (jsonObject, xhr) {
            if (this.isDataSet()) {
                if (this._dataProperty) {
                    ...
                    for (var buf in this._buffers) {
                        var data;
                        if (jsonObject[this._dataSetName]) data = jsonObject[this._dataSetName][buf];else data = null;
                        data = data ? data : [];
                    ...
                    if (this._numBuffers > 1) {
                        for (var buf in this._buffers) {
                            if (this._buffers[buf]._isNested && this._buffers[buf]._parent && this._buffers[this._buffers[buf]._parent]) {
                ...
                if (jsonObject instanceof Array) {
                    ...
                    if (this._dataProperty) this._defaultTableRef._data = jsonObject[this._dataProperty];else if (jsonObject.data) this._defaultTableRef._data = jsonObject.data;else {
        ...
        this._arrayFromDataObject = function (dataObject, tableRef) {
            ...
            if (this._dataSetName) {
                if (dataObject[this._dataSetName]) data = dataObject[this._dataSetName][tableRef._name];
            } else {
            ...
            if (this._dataSetName) {
                ...
                if (tableJsonObject instanceof Array) {
                    ...
                    for (var i = 0; i < tableJsonObject.length; i++) {
                        ...
                        if (errorString) this._setErrorString(tableRef, recordId, errorString, false);

                        // Set _rejected property
                        if (tableJsonObject[i]["prods:rejected"] || errorString) {
        ...
        this._mergeUpdateForSubmit = function (jsonObject, xhr) {
            ...
            var dataSetJsonObject = jsonObject[this._dataSetName];
            if (dataSetJsonObject[this._dataSetName]) dataSetJsonObject = dataSetJsonObject[this._dataSetName];
                    ...
                    if (beforeTableJsonObject instanceof Array) {
                        for (var i = 0; i < beforeTableJsonObject.length; i++) {
                            ...
                            if (beforeTableJsonObject[i]["prods:rowState"] == "deleted") {
                                ...
                                // Set _rejected property so it can be checked in applyChanges()
                                if ((beforeTableJsonObject[i]["prods:rejected"] || errorString) && tableRef._beforeImage[recordId]) {
            ...
            if (tableJsonObject instanceof Array) {
                for (var i = 0; i < tableJsonObject.length; i++) {
                    ...
                    if (id === prods_id) {
                        ...
                        return errorString === null ? "Server returned unspecified error. Please check log files." : errorString;
                    }
                }
            }
        ...
        this._undoUpdate = function (tableRef, id, deleteProdsProps) {
            ...
            if (record) {
                ...
                if (deleteProdsProps) tableRef._jsdo._deleteProdsProperties(tableRef._data[index], true);
            }
            delete tableRef._beforeImage[id];
        ...
        this._undoDelete = function (tableRef, id, deleteProdsProps) {
            ...
            if (record) {
                ...
                if (deleteProdsProps) tableRef._jsdo._deleteProdsProperties(record, true);
        ...
        this._deleteSuccess = function (jsdo, success, request) {
            ...
            if (hasError) request.success = false;
        ...
        this._saveChangesComplete = function (jsdo, success, request) {
            // Success with errors
            if (request.xhr.status >= 200 && request.xhr.status < 300 && (jsdo._lastErrors.length > 0 || jsdo._someRecordsRejected)) {
                request.success = false;
            }
        ...
        this._getErrorsFromRequest = function (request) {
            var errors = [],
                // Array of objects with properties: type, id, error, errorNum, responseText
            errorArray = [],
                errorObject,
                retValString,
                j,
            ...
            if (request && !request.success) {
                        ...
                        for (i = 0; i < errorArray.length; i += 1) {
                            ...
                            if (errorObject._errors instanceof Array) {
                                for (j = 0; j < errorObject._errors.length; j += 1) {
                                    if (errorObject._errors[j]._errorNum === 0 && errorObject._errors[j]._errorMsg === retValString) {
                ...
                if (errors.length === 0 && request.xhr && request.xhr.status >= 400 && request.xhr.status < 600) {
                    errors.push({
                        type: progress.data.JSDO.ERROR,
        ...
        this._updateLastErrors = function (jsdo, batch, changes, request) {
            var errors, errorText, responseObject, i, j, buf;
                    ...
                    if (!request.success && request.xhr) {
                        if (request.xhr.status >= 200 && request.xhr.status < 300) {
                            ...
                            for (j = 0; j < errors.length; j += 1) {
                                ...
                                if (request.jsrecord && (errors[j].type === progress.data.JSDO.APP_ERROR || errors[j].type === progress.data.JSDO.RETVAL)) {
            ...
            } else if (request && !request.success && request.xhr && (request.xhr.status >= 400 && request.xhr.status < 600 || request.xhr.status === 0)) {
                errors = this._getErrorsFromRequest(request);
                errorText = "";
                for (j = 0; j < errors.length; j += 1) {
                    if (errors.length > 1 && errors[j].error.indexOf("(7243)") != -1) {
        ...
        this._mergeInvoke = function (jsonObject, xhr) {
            var operation;
            if (xhr.request.fnName !== undefined && xhr.jsdo._resource.fn[xhr.request.fnName] !== undefined) {
                    ...
                    for (var i = 0; i < operation.params.length; i++) {
                        if (operation.params[i].name == dataParameterName) {
                            if (operation.params[i].type.indexOf('RESPONSE_BODY') != -1) {
                                if (operation.params[i].xType !== undefined && operation.params[i].xType != 'DATASET' && operation.params[i].xType != 'TABLE' && operation.params[i].xType != 'ARRAY') {
                ...
                try {
                    if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 0 && xhr.responseText !== "") {
                        ...
                        if (typeof xhr.onSuccessFn == 'function') {
                            ...
                            if (operation === undefined || operation !== null && operation.mergeMode) xhr.jsdo._mergeInvoke(request.response, xhr);
                            if (request.success) xhr.onSuccessFn(xhr.jsdo, request.success, request);else if (typeof xhr.onErrorFn == 'function') xhr.onErrorFn(xhr.jsdo, request.success, request);
                        }
                    } else {
                        request.success = false;
                        if (xhr.status === 0) {
        ...
        this.hasChanges = function () {
            for (var buf in this._buffers) {
                if (this._buffers[this._buffers[buf]._name].hasChanges()) return true;
            }
            return false;
        };
         ...
         */
        this.acceptRowChanges = function () {
            if (this._defaultTableRef) return this._defaultTableRef.acceptRowChanges();
         ...
         */
        this.rejectRowChanges = function () {
            if (this._defaultTableRef) return this._defaultTableRef.rejectRowChanges();
        ...
        this.setProperties = function (propertiesObject) {
            ...
            if ((typeof propertiesObject === "undefined" ? "undefined" : _typeof(propertiesObject)) == "object") {
                ...
                // {1}: Parameter {1} must be of type {3} in {4} call.
                throw new Error(progress.data._getMsgText("jsdoMSG121", 'JSDO', 1, 'Object', 'setProperties'));
            }
        };
        ...
        this.setProperty = function (propertyName, propertyValue) {
            if (arguments.length < 2) {
                // {1}: Incorrect number of arguments in {2} call. There should be {3}.
                throw new Error(progress.data._getMsgText("jsdoMSG122", 'JSDO', 'setProperty', 2));
            }
            if (arguments.length !== 2) {
                // {1}: Incorrect number of arguments in {2} call. There should be only {3}.";
                throw new Error(progress.data._getMsgText("jsdoMSG122", "JSDO", "setProperty", 2));
            ...
            if (typeof propertyName !== "string") {
                // {1}: Parameter {1} must be of type {3} in {4} call.
                throw new Error(progress.data._getMsgText("jsdoMSG121", 'JSDO', 1, 'string', 'setProperty'));
            }
        ...
        this.readLocal = function readLocal(name) {
            ...
            if (object) {
                if (this._hasMatchingSchema(object) === false) throw new Error(msg.getMsgText("jsdoMSG117", name));
        ...
        this._containsPrimaryKeys = function _containsPrimaryKeys() {
            ...
            for (var buf in this._buffers) {
                if (this._buffers[buf]._primaryKeys === null) return false;
            }
        ...
        this._hasMatchingSchema = function _hasMatchingSchema(storageObject) {
            ...
            if (storageObject === null || Object.keys(storageObject).length === 0) return true;
            ...
            if (this._dataSetName) {
                ...
                } else isValid = false; // dataset should be in storage area
            } else if (this._dataProperty) {
                // If array, we had to wrap in "fake" dataset, so unwrap it
                storageObject = storageObject["_localStorage"];
                if (storageObject === undefined || storageObject[this._dataProperty] === undefined) isValid = false;
            } else {
                // If temp-table, we had to wrap in "fake" dataset, so unwrap it
                storageObject = storageObject["_localStorage"];
                if (storageObject === undefined || storageObject[this._defaultTableRef._name] === undefined) isValid = false;
            }
        ...
        this._restoreFromLocalStorage = function (storageObject, addMode, keyFields) {
            ...
            if (storageObject && Object.keys(storageObject).length > 0) {
                ...
                else this._restoreDataForTable(this._defaultTableRef, storageObject, addMode, keyFields);
            } else if (addMode === progress.data.JSDO.MODE_EMPTY) this._clearData();
        };
    ...
    progress.data.PluginManager.addPlugin("JFP", {
        requestMapping: function requestMapping(jsdo, params, info) {
            var sortFields,
                field,
                fieldName,
                fieldInfo,
            ...
            if (info.operation === "read") {
                ...
                for (param in params) {
                    if (param && params[param] !== undefined && reqCapabilities[param]) {
                        ...
                        if (!reqCapabilities[param].mapping) {
                            throw new Error(msg.getMsgText("jsdoMSG120", reqCapabilities[param].options.join("' or '"), param));
                        }
                    }
                }
                ...
                if (params.sort) {
                    ...
                    if (_typeof(params.sort) === "object" && !(params.sort instanceof Array)) {
                ...
                if (params.filter) {
                    ...
                    if (doConversion && params.tableRef === undefined) {
                        throw new Error(msg.getMsgText("jsdoMSG045", "fill() or read()", "params", "tableRef"));
                    }

                    if (reqCapabilities["filter"].mapping === "ablFilter") {
                        if (doConversion) {
                            ablFilter = progress.util._convertToABLWhereString(jsdo._buffers[params.tableRef], params.filter);
                        } else {
                            ablFilter = params.filter;
                        }
                    } else if (reqCapabilities["filter"].mapping === "sqlQuery") {
                        if (doConversion) {
                            sqlQuery = progress.util._convertToSQLQueryString(jsdo._buffers[params.tableRef], params.filter, true);
                        } else {
    ...
    if (typeof progress.ui == 'undefined') progress.ui = {};
    progress.ui.UITableRef = function UITableRef(tableRef) {
        ...
        this.addItem = function (format) {
            ...
            if (!this._tableRef.record) throw new Error(msg.getMsgText("jsdoMSG002", this._name));
            ...
            for (var field in this._tableRef.record.data) {
                ...
                text = text.replace(new RegExp('{' + field + '}', 'g'), value !== undefined && value !== null ? value : "");
            }
        ...
        this._getFormFieldValue = function (fieldName, detailPageName) {
            ...
            if (detailPageName === undefined) {
                if (this._detailPage && this._detailPage.name) detailPageName = this._detailPage.name;
            }

            if (typeof $ == 'function' && detailPageName) {
                field = $("#" + detailPageName + " #" + fieldName);
                if (!field || field.length === 0) field = $("#" + detailPageName + ' [dsid="' + fieldName + '"]');
                if (field && field.length == 1) value = field.val();
            } else {
        ...
        this._setFormField = function (fieldName, value, detailPageName) {
            ...
            if (typeof $ == 'function' && detailPageName) {
                field = $("#" + detailPageName + " #" + fieldName);
                if (!field || field.length === 0) field = $("#" + detailPageName + ' [dsid="' + fieldName + '"]');
                if (field && field.length == 1) field.val(value);
            } else {
                field = document.getElementById(fieldName);
                if (field) {
                    field.value = value;
        ...
        this.assign = function (detailPageName) {
            if (!this._tableRef.record) throw new Error(msg.getMsgText("jsdoMSG002", this._tableRef._name));
            if (arguments.length !== 0 && typeof detailPageName != 'string') throw new Error(msg.getMsgText("jsdoMSG024", "UIHelper", "assign()"));
        ...
        this.showListView = function () {
            ...
            if (typeof $ == 'function') {
                ...
                if (listviewElement && listviewElement.length == 1) {
                    ...
                    try {
                        if (listviewElement.attr("data-filter") === "true" && typeof listviewElement.filterable === "function") {
                ...
                if (this._listview.autoLink) {
                    ...
                    $("#" + this._listview.name + " li").each(function () /* index */{
        ...
        this.getFormFields = function (fields) {
            if (!this._tableRef._schema) return '';
            if (!(fields instanceof Array)) fields = null;else {
                var tmpFields = {};
                for (var i = 0; i < fields.length; i++) {
        ...
        this.setDetailPage = function setDetailPage(obj) {
            if (!obj || (typeof obj === "undefined" ? "undefined" : _typeof(obj)) != 'object') throw new Error(msg.getMsgText("jsdoMSG012", arguments.callee.name, "object"));
            if (!obj.name || typeof obj.name != 'string') throw new Error(msg.getMsgText("jsdoMSG012", arguments.callee.name, "name"));
        ...
        this.setListView = function setListView(obj) {
            if (!obj || (typeof obj === "undefined" ? "undefined" : _typeof(obj)) != 'object') throw new Error(msg.getMsgText("jsdoMSG012", arguments.callee.name, "object"));
            if (!obj.name || typeof obj.name != 'string') throw new Error(msg.getMsgText("jsdoMSG012", arguments.callee.name, "name"));
            if (obj.format && typeof obj.name != 'string') throw new Error(msg.getMsgText("jsdoMSG012", arguments.callee.name, "format"));
    ...
    progress.ui.UIHelper = function UIHelper() {
        ...
        for (var buf in this._jsdo._buffers) {
            ...
            if (!this._defaultUITableRef) this._defaultUITableRef = this._uiTableRef[buf];
            cnt++;
        }
        if (cnt != 1) {
            this._defaultUITableRef = null;
        ...
        this.addItem = function (format) {
            ...
            } else throw new Error(msg.getMsgText("jsdoMSG011", "addItem()"));
        };

        this.clearItems = function () {
            if (this._defaultUITableRef) {
                this._defaultUITableRef.clearItems();
            } else throw new Error(msg.getMsgText("jsdoMSG011", "clearItems()"));
        };

        this.assign = function (detailPageName) {
            if (arguments.length !== 0) throw new Error(msg.getMsgText("jsdoMSG024", "UIHelper", "assign()"));
            if (this._defaultUITableRef) {
                return this._defaultUITableRef.assign(detailPageName);
            } else throw new Error(msg.getMsgText("jsdoMSG011", "assign()"));
        };

        this.display = function (detailPageName) {
            if (this._defaultUITableRef) {
                this._defaultUITableRef.display(detailPageName);
            } else throw new Error(msg.getMsgText("jsdoMSG011", "display()"));
        };

        this.showListView = function () {
            if (this._defaultUITableRef) {
                this._defaultUITableRef.showListView();
            } else throw new Error(msg.getMsgText("jsdoMSG011", "showListView()"));
        };

        this.getFormFields = function (fields) {
            if (this._defaultUITableRef) {
                return this._defaultUITableRef.getFormFields(fields);
            } else throw new Error(msg.getMsgText("jsdoMSG011", "getFormFields()"));
        };

        this.getListViewRecord = function (htmlIElement) {
            if (this._defaultUITableRef) {
                return this._defaultUITableRef.getListViewRecord(htmlIElement);
            } else throw new Error(msg.getMsgText("jsdoMSG011", "getListViewRecord()"));
        };

        this.getFormRecord = function (detailPageName) {
            if (this._defaultUITableRef) {
                return this._defaultUITableRef.getFormRecord(detailPageName);
            } else throw new Error(msg.getMsgText("jsdoMSG011", "getFormRecord()"));
        };

        this.setDetailPage = function (obj) {
            if (this._defaultUITableRef) return this._defaultUITableRef.setDetailPage(obj);
            throw new Error(msg.getMsgText("jsdoMSG011", "setDetailPage()"));
        };

        this.setListView = function (obj) {
            if (this._defaultUITableRef) return this._defaultUITableRef.setListView(obj);
...
(function () {
    ...
    progress.data.AuthenticationProvider = function (initObject) {
        var authProv, authModel, uri;

        // process constructor arguments
        if ((typeof initObject === "undefined" ? "undefined" : _typeof(initObject)) === 'object') {

            // these 2 calls throw an appropriate error if the check doesn't pass
            this._checkStringArg("constructor", initObject.authenticationModel, "initObject.authenticationModel", "initObject.authenticationModel");

            this._checkStringArg("constructor", initObject.uri, "init-object.uri", "init-object.uri");
        } else {
            // AuthenticationProvider: Invalid signature for constructor. The init-object argument 
            //                         was missing or invalid.
            throw new Error(progress.data._getMsgText("jsdoMSG033", "AuthenticationProvider", "the constructor", "The init-object argument was missing or invalid."));
        }

        authModel = initObject.authenticationModel.toLowerCase();
        switch (authModel) {
            case progress.data.Session.AUTH_TYPE_ANON:
                this._initialize(initObject.uri, progress.data.Session.AUTH_TYPE_ANON, { "_loginURI": progress.data.AuthenticationProvider._homeLoginURIBase });
                authProv = this;
                break;
            case progress.data.Session.AUTH_TYPE_BASIC:
            ...
            default:
                ...
                throw new Error(progress.data._getMsgText("jsdoMSG502", "AuthenticationProvider", "init-object", "constructor", "authenticationModel"));
            //break;
        }
    ...
    progress.data.AuthenticationProvider.prototype._loginProto = function (sendParam) {
        var deferred = $.Deferred(),
            xhr,
            uriForRequest,
            header,
    ...
    // it to use promises
    progress.data.AuthenticationProvider.prototype._openRequestAndAuthorize = function (xhr, verb, uri, async, callback) {
        var errorObject;
    ...
    // General purpose utility method, no overrides expected
    progress.data.AuthenticationProvider.prototype._checkStringArg = function (fnName, argToCheck, argPosition, argName) {
        ...
        if (typeof argToCheck !== "string") {
            ...
            throw new Error(progress.data._getMsgText("jsdoMSG121", "AuthenticationProvider", argPosition, "string", fnName));
        } else if (argToCheck.length === 0) {
            //  AuthenticationProvider: {param-name} cannot be an empty string.
            throw new Error(progress.data._getMsgText("jsdoMSG501", "AuthenticationProvider", argName, fnName));
        }
    };
